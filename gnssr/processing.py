...
import gnssr.elv_interp_txt
import gnssr.tropd
from gnssr.helper import (
    readsnrtxt, datetime2gps, 
    gps2datetime, glonasswlen, 
    cubspl_nans, residuals_cubspl_spectral,
)
from gnssr.plot import plotrhspline
import time


def snr2arc(snrdata, rhlims, gsignal='L1', polydeg=2, gaptlim=5*60, detrend=True, **kwargs):
    """
...
def snr2arcs(sdt, edt, snrdir, arcdir, rhlims, arclim, iterdt_arcs, antennaids=[''], **kwargs):
...
def snr2arc_iterate(sdt, edt, snrdir, arcdir, rhlims, arclim, iterdt_arcs, snrfilelen=False, gsignal='L1',
                    elvinterp=True, kdt_elvinterp=1800, offset_elvinterp=1800, **kwargs):
    """
    ...
    while tdt < edt - iterdt_td:
        ...
        tfilter = np.logical_and(snrdata[:, 3] >= tgts_interp,
                                 snrdata[:, 3] < tgte)
        snrdata = snrdata[tfilter]
        ...
        if elvinterp:
            ...
            timey = time.time()
            snrdata = gnssr.elv_interp_txt.elv_interp_array(snrdata, kdt_elvinterp)
            tfilter = np.logical_and(snrdata[:, 3] >= tgts,
                                     snrdata[:, 3] < tgte)
            print(f"took %.1f seconds to interpolate elv " % (time.time()-timey))
            snrdata = snrdata[tfilter]
            if np.ma.size(snrdata, 0) == 0:
                print('mssing data')
                continue
        ...
        print(f"took %.2f seconds to convert to arcs " % (time.time()-timey))
...
def collectarcs(arcdir, sdt, edt, hgts, antennaids, collectsnr=False, arclim=24*60*60, **kwargs):
...
def arcs2spline(rh_arr, snrdt_arr, knots, doplot=False, tropd_adj=True, **kwargs):
    ...
    if tropd_adj:
        if 'lla' and 'gptfile' in kwargs:
            tdt = np.median(knots)
            rh_fac, rh_arr_adj = gnssr.tropd.tropd_adj(tdt, rh_arr, **kwargs)
            rh_arr = rh_arr_adj
...
def arcs2splines(arcdir, sdt, edt, iterdt_spline, kdt, prek, postk, hgts, antennaids, 
                 arclim=60*60, savespline=False, **kwargs):
    """
    """     
    ...
    while tdt < datetime2gps(edt) - iterdt_spline:
        ...
        rh_arr, snrdt_arr = collectarcs(arcdir, gps2datetime(knots[0]), gps2datetime(knots[-1]), hgts, antennaids,
                                        arclim=arclim, **kwargs)
        ...
        rh_arr = rh_arr[np.logical_and(rh_arr[:, 0] > tdt - iterdt_spline, rh_arr[:, 0] <= tdt + iterdt_spline), :]
        if len(invout) == 0:
            kval_spectral = np.append(kval_spectral, np.nan)
            continue
