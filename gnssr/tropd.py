...
import pickle
import numpy as np
from gnssr.make_gpt import makegptfile, gpt2_1w

# written by David Purnell

def tropd_adj(gpst, rh_arr, iminelv=4, imaxelv=5, irh=1, iahgt=12, it=0, adjtype='allelv', **kwargs):
    """
    ...
    if adjtype == 'minmaxelv':
        ...
        rh_fac = corr_rh_facs(np.min(rh_arr[:, iminelv]), np.max(rh_arr[:, imaxelv]), pant, tant, eant)
        rh_arr_adj = rh_arr
...
def relhumtemp2e(relhum, temp):
    ...
    """
    tempk = temp + 273.16
    log10svp = 10.79574 * (1 - 273.16 / tempk) - 5.028 * np.log10(tempk / 273.16) + \
               1.50475 * 10 ** -4 * (1 - 10 ** (-8.2969 * (tempk / 273.16 - 1))) + \
               0.42873 * 10 ** -3 * (10 ** (-4.76955 * (1 - 273.16 / tempk) - 1)) + 0.78614
    svp = 10 ** log10svp
    e = relhum / 100 * svp
    return e
...
def bend_eqn(p, t, elv):
    bending_corr_arc_min = 510 / (9 / 5 * t + 492) * p / 1010.16 * 1 / np.tan(
        np.deg2rad(elv + 7.31 / (elv + 4.4)))
